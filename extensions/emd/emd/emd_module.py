...
class emdFunction(Function):
    @staticmethod
    def forward(ctx, xyz1: torch.Tensor, xyz2: torch.Tensor, eps: float, iters: int):
        ...
        emd_cuda.forward(xyz1, xyz2, dist, assignment, price, assignment_inv, bid, bid_increments, max_increments, unass_idx, unass_cnt, unass_cnt_sum, cnt_tmp, max_idx, eps, iters)
    ...
    def backward(ctx, graddist, gradidx):
        ...
        emd_cuda.backward(xyz1, xyz2, gradxyz1, graddist, assignment)
...
def test_emd():
    x1 = torch.rand(20, 4096, 3).cuda() # please normalize your point cloud to [0, 1]
    x2 = torch.rand(20, 4096, 3).cuda()
    print(x1.min(), x1.max())
    emd = emdModule()
    ...
    x2 = np.take_along_axis(x2, assigment, axis = 1) # type: ignore
    d = (x1 - x2) * (x1 - x2)
